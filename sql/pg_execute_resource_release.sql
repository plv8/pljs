...
--
-- A number leaked for a second reason, whether it arrived as a NUMERIC column
-- or inside a JSONB value: the conversion called numeric_float8(), which as of
-- PostgreSQL 19 frees the string it renders only on its error path.  pljs
-- renders and frees that string itself now.  Both shapes are covered below.
...
SELECT result_context_bytes(5000, q) < result_context_bytes(100, q) * 4
  ...
  FROM (VALUES ('SELECT repeat(''x'', 120)::text AS v'),
               ...
               ('SELECT repeat(''a'', 120)::bytea AS v'),
               ('SELECT 1.5::numeric AS v')) AS t(q);
